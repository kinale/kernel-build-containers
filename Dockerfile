...
RUN set -ex; \
    if [ "$GCC_VERSION" ]; then \
      ...
      if [ "$GCC_VERSION" != "4.9" ]; then \
        apt-get install -y -q --no-install-recommends gcc-${GCC_VERSION}-powerpc64-linux-gnu g++-${GCC_VERSION}-powerpc64-linux-gnu; \
        apt-get install -y -q --no-install-recommends gcc-${GCC_VERSION}-plugin-dev-aarch64-linux-gnu \
